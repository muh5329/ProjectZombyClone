...
func test_player_in_vision_cone_is_chased_within_1s() -> void:
	...
	check(z.visual.head_material().albedo_color.r > 0.7, "red head while hostile")
	await frames(1)
...
func test_zombie_death_leaves_interactable_corpse() -> void:
	...
	check_near(vis.rotation.x, -PI * 0.5, 0.01, "collapsed visual")
	check(vis.head_material().albedo_color.r < 0.3, "dark head when dead")
