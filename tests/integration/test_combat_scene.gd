...
func test_knockdown_down_multiplier_and_get_up() -> void:
	...
	check_eq(z.visual.clip(), &"z_knockdown", "fall clip playing (R8.5)")
	check(await wait_physics_until(func(): return z.visual.is_lying(), 60), "model lies on the ground")
	var hp := z.health()
	...
	check_eq(z.visual.clip(), &"z_getup", "get-up clip (R8.5)")
	check(await wait_physics_until(func(): return not z.visual.is_lying(), 60), "standing again")
