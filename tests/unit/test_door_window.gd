...
func test_door_actions_and_state() -> void:
	...
	check_eq(_ids(it.get_actions(_actor)), [&"open", &"barricade"], "closed door offers Open (+ Barricade, R9)")
	check_eq(_ids(it.get_actions(_actor), true), [&"open"], "barricade disabled without a hammer")
...
func test_window_closed_actions() -> void:
	...
	check_eq(_ids(acts), [&"open", &"smash", &"climb", &"barricade"], "closed: open, smash, climb(disabled), barricade (R9)")
...
func test_window_open_and_close() -> void:
	...
	var acts := it.get_actions(_actor)
	check_eq(_ids(acts), [&"climb", &"close", &"smash", &"barricade"], "open: climb first (E), then close, smash, barricade")
...
func test_window_smashed_actions() -> void:
	...
	check_eq(_ids(acts), [&"climb", &"clear_glass", &"barricade"], "smashed: climb + remove glass (+ barricade)")
